...
export class DataPipelineListPage {
    ...
    async clickActionMenu(uniqueText: string): Promise<ElementFinder> {
        ...
        browser.wait(EC.visibilityOf(menuList), 3000, 'Action menu not visible after wait of 3 seconds').catch(() => {});
        return menuList;
    }
    ...
    async deleteRecord(uniqueText: string): Promise<DataPipelineDeleteDialogPage> {
        ...
        await browser.wait(EC.visibilityOf(menuList), 6000, 'Delete option not visible after wait of 6 seconds').catch(() => {});
